...
function getRectangleString(width, height) {
  ...
  const strTop = `┌${strMiddle}┐\n`;
  const strCenter = `│${' '.repeat(width - 2)}│\n`;
  const strBottom = `└${strMiddle}┘\n`;
  return `${strTop}${strCenter.repeat(height - 2)}${strBottom}`;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const index = entry.indexOf(str[i]);
    if (index !== -1) newString += exit[index];
